...
| 6. 実例確認の題材 | **ローカルGitLabに演習用プロジェクトを1つ作り、実投稿まで確認する**（flow-id 2-9でレビュー決定） |

**この調査で確かめられなかったこと**（詳細は各論点と末尾「未決定事項」）:
3000ファイル超・巨大ファイルでの `pulls/<n>/files` の挙動、GitLab側で差分が
truncate/collapse される規模での挙動、両プロバイダでのリネーム時の挙動。
**実際の演習MRに対する動作（投稿まで）は、フェーズ3でローカルGitLabの演習用プロジェクトを
使って確認する**（論点6・flow-id 2-9で決定）。
...
→ **題材はこのリポジトリに置かず、ローカルGitLab上の演習用プロジェクトとして作る**
（下記「投稿まで確認する」）。確認結果だけを `reports/` へ記録する。

### 投稿まで確認する（flow-id 2-9で決定）

当初は「演習用リポジトリが無く、実投稿の対象が本PR #2 しかないため、**findings確定・ネタバレ検査
通過まで**を確認範囲とし投稿は行わない」と提案していた。**レビューで「演習用プロジェクトを1つ
作って実投稿まで確認する」と決まった**ため、この提案は取り下げる。

**確認の場はローカルGitLab**（`localhost:8929`）とする。論点2の検証で、このインスタンスが
使えること・`glab` が `root` として認証済みであること・過去の検証用プロジェクトが同じやり方で
作られていること（`root/issue45-verify` 等）を確認済みである。GitHub側の公開リポジトリへ
ダミー指摘を残さずに済む点でも、こちらが適している。

用意するもの（フェーズ3の検証手順として組み立てる）。

| 要素 | 内容 |
|---|---|
| 演習用プロジェクト | ローカルGitLabに1つ作る（例: `root/answer-talker-verify`） |
| default ブランチ | 演習の初期状態 |
| 受講者のMR | **1ファイルに詰め込んだ実装**を載せたブランチ → default へのMR |
| 正解ソース | **同じ機能を3ファイルへ分割**したもの。論点3の2形態（URL clone／ローカルパス）を両方試せるよう、別プロジェクトとして置いたうえでローカルにも展開する |

これにより、次がまとめて確認できる。

- 受け入れ条件「正解側にしか存在しない分割単位について、**実例で確認**」（論点4・6）
- ネタバレ検査が働き、混入した finding が投稿されないこと（論点5）
- **GitLab経路の投稿処理** `gitlab_add_mr_inline_comments`（`Gitlab.sh:360`）が実MRに対して
  動くこと。論点2で確認したのは差分の**取得**までであり、投稿は未確認だった
- 論点3の正解ソース取得を、URL形態・ローカルパス形態の両方で

**投稿先が演習用の使い捨てプロジェクトなので、指摘の内容が不適切でも実害が無い。** 逆に言えば、
ここで実投稿まで通しておかないと、投稿経路が動くことをどこでも確認しないまま完了することになる。
...
| ~~6~~ | 6 | ~~実例確認で実際にMRへ投稿するか~~ → **解消**（flow-id 2-9）。**ローカルGitLabに演習用プロジェクトを1つ作り、実投稿まで確認する** |
