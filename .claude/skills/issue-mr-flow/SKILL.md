...
- **2周目以降に入るときは、作業を始める前に`add-round <flow-id>`を呼ぶ。** 周回数が1つ進み、
  範囲の記号が`[]`へ戻る。
- **フェーズを丸ごともう1周するときは、`add-round`だけでは足りない**（issue #6で2度踏んだ）。
  1つのフェーズで個別計画を複数持つ場合（`【設計】`のあとに`【実装】【テスト】`、`【設計反映】`の
  あとに`【AIアセット反映】`など）、2本目に着手する時点でループ範囲だけでなく**同じフェーズの
  単発ステップ**（3-1・3-2・3-5・3-10／4-10 など）もやり直しになる。`add-round`が`[]`へ戻すのは
  ループ範囲だけなので、**単発ステップの記号は手作業で`[]`へ戻す**。戻し忘れると「description更新
  まで終わっている」という誤った状態が、次の1周のあいだ表に残り続ける。
...
## 演習MRレビューの位置づけ（issue #1）

`answer-talker` スキル（`.claude/skills/answer-talker/SKILL.md`）は、**別途用意された正解ソースと
照合して**演習MRをレビューし、指摘をMRへインラインコメントとして投稿する。敵対的レビューと同じく
**上の全体フロー表には含まれない**（flow-idを1つも増やしていない）。

| | 内容 |
|---|---|
| 位置 | フローの外。**このリポジトリの開発フローの一部ではなく、演習の講師が使う独立した道具**である |
| 対象 | **引数で受けた他人のMR番号**（別リポジトリでもよい）。実行者のブランチとは無関係 |
| 起動 | 人間が `/answer-talker <MR番号> --reference <正解の場所>` を呼んだときだけ実行する |
| 回数 | **上限機構を持たない**（`.claude/docs/ddr/0058-answer-talkerには実施回数の上限機構を課さない.md`） |
| 進捗記号 | `HANDOFF.md` の進捗表は動かさない（flow-idを持たないため） |

**`adversarial-review` と混同しない。** 2つは投稿経路を共有するが、目的が違う。

| | `adversarial-review` | `answer-talker` |
|---|---|---|
| 何のため | このリポジトリの変更が壊れないか | 演習の受講者の設計の考え方を正解と同じ方向へ向ける |
| 対象 | **自分のブランチ**のMR | **引数で受けた他人のMR** |
| フローとの関係 | フローの各レビューの**前に挟む補助** | **フローの外**（開発フローとは無関係） |
| 投稿ラベル | `敵対的レビュー` | `演習レビュー` |

仕様は `.claude/docs/spec/answer-talker.md` を正とする。
