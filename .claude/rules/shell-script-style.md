...
- **大きなJSONを`--argjson`/`--arg`等のコマンドライン引数としてjqへ渡さない**（issue #37対応時に
  ...
  - **`--args` による位置引数も同じ上限を共有する**（issue #6で実際に踏んだ）。上の実例は
    `--argjson` だが、可変長のパス一覧を `jq --args ... -- "${paths[@]}"` の形で渡す場合も
    同じところで壊れる。**実測の境界は500件・平均パス長64文字**（合計およそ32KB＝Windowsの
    コマンドライン長上限）で、そこで `jq: Argument list too long` になった。
    「JSONを渡していないから安全」ではない。
  - **どこから壊れるかは呼び出し元の上限で決まる。** 上の実例が「数十KB」、この実例が
    「500件」と見え方が違うのは、どちらもコマンドライン全体のバイト数という同じ上限へ
    別の経路で達しているだけである。**要素数に上限を設けているスクリプトでは、その上限値
    そのものが境界を踏まないかを確かめる**（issue #6 では一覧の上限が500件で、上限どおりに
    埋まったときには必ず失敗する状態だった。上限が小さいから安全なのではなく、上限が
    たまたま境界の手前にあっただけ、ということがある）。
  - 受け口は `--rawfile` にする。**空のときに `[""]` ではなく `[]` になるよう明示的に
    分岐すること**（`rtrimstr` の結果が空文字列でも `split` は要素1個の配列を返すため）。

    ```bash
    # 悪い例（件数が上限まで埋まると Argument list too long で落ちる）
    jq -nc --args '{files: $ARGS.positional}' -- "${paths[@]}"
    # 良い例（ファイル経由。件数に依存しない）
    printf '%s\n' "${paths[@]}" > "$list_file"
    jq -nc --rawfile listRaw "$list_file" \
      '{files: ($listRaw | rtrimstr("\n") | if . == "" then [] else split("\n") end)}'
    ```

    実例は `.claude/scripts/src/answer-talker-submission.sh` の `resolve_main`。
- **上記の失敗が別の関数へ波及して恒久化するケースに注意する**: あるjq呼び出しが上記の理由で
  ...
  - **`jq -e .`は空文字列の入力に対して失敗を検知できないことがある**（実機確認:
    ...
    `jq -e .`の判定より先に行うこと。
- **jqの予約語を`--arg`/`--argjson`の変数名に使わない**（issue #1で実際に踏んだ）。`label`は
  jqの予約語（`label $out | ...` 構文）であり、`--arg label "$x"` と書くと
  `jq: error: syntax error, unexpected label` になる。**エラーはフィルタの構文エラーとして出る
  ため、原因が変数名にあると気づきにくい**。変数名は自然な英単語を選びがちで、他の予約語
  （`def` `reduce` `foreach` `import` `include` `as` `and` `or` `not` `if` `then` `elif` `else`
  `end` `try` `catch` `__loc__`）でも同じことが起きる。用途を足して衝突を避ける
  （実例: `--arg label` → `--arg reviewLabel`）。
...
- **`grep -rlI -- '' <dir>` の結果の補集合を「バイナリ」とみなさない**（issue #6で実際に踏んだ）。
  この書き方はファイルごとに判定コマンドを起動しない点では正しいが、**0バイトのファイルが
  補集合に落ちる**。grepは行単位でマッチするため、行が1つも無いファイルはどのパターンにも
  マッチせず列挙されないというだけであり、**`-I` によるバイナリ判定とは無関係**である
  （実機確認: `-I` を外しても空ファイルは列挙されない）。

  ```bash
  # empty.py は0バイト、bin.dat はNULを含む、noeol.txt は末尾に改行が無い
  grep -rlI -- '' .   # → ./noeol.txt ./text.txt           （empty.py が落ちる）
  grep -rl  -- '' .   # → ./bin.dat ./noeol.txt ./text.txt （-I を外しても empty.py は落ちる）
  ```

  **末尾に改行が無いだけのファイルは列挙される**ので、落ちるのは行が1つも無いファイルに限られる。
  `__init__.py` `.gitkeep` 空の設定ファイルは普通に存在するため、補集合をそのまま削除・除外の
  対象にすると**実在するファイルが静かに消える**。`find <dir> -type f -empty` で補う。

  ```bash
  # 悪い例（空ファイルがバイナリとして削除される）
  comm -13 <(grep -rlI -- '' "$root" | sort) <(find "$root" -type f | sort) | xargs -r rm -f
  # 良い例1（削除側: 空ファイルを走査対象から外す）
  comm -13 <(grep -rlI -- '' "$root" | sort) <(find "$root" -type f ! -empty | sort) | xargs -r rm -f
  # 良い例2（列挙側: 空ファイルを足す）
  { grep -rlI -- '' "$root"; find "$root" -type f -empty; } | sort -u
  ```

  `-size 0` でも同じ結果になるが（実機確認）、`-size` の既定単位は512バイトブロックであり
  数値だけでは意図が読み取りにくい。**空であることを問うなら `-empty` を使う。**

- **削除側と列挙側の両方でこの判定を使っている場合、片方だけを直さない。** ツリーからは消して
  いないのに一覧に載らない（またはその逆の）ファイルが生まれる。issue #6 の実装では、
  一覧に無いファイルはサブエージェントが**確認そのものを行わない**ため、片側だけの修正は
  「存在するのに無いことになっているファイル」を作る。
...
- Windows版のnative `jq`バイナリ（`C:\Program Files\jq\jq.exe`のような、MSYS版ではなくWindows
  ...
  - **`jq -r`で「複数行の値」を取り出したときは、行の途中にCRが残る**（issue #1で実際に踏んだ）。
    上と同じCR付与だが、**単一行の値では表面化しない**点が違う。コマンド置換が落とすのは
    末尾の改行だけなので、`"a\nb\nc"`を`$(... | jq -r ...)`で受けると内部の`\r`が2個残って
    7バイトになる（実機確認。`"PostToolUse"`のような単一行は11バイトでCRが残らない）。
    このため、**同じ関数を検査するテストでも、単一行のアサーションは通り複数行のアサーションだけが
    `行数-1`バイトずれて失敗する**という形になり、「見た目は同じなのに一致しない」と見える。
    複数行の値を`jq -r`で取り出す箇所には`| tr -d '\r'`を挟む。
...
- **「defaultブランチでも同じテストが失敗する」ことを、プロダクション側の不具合の証拠にしない**
  （issue #1で実際に踏んだ）。`git worktree`で`origin/main`を切り出して同じ失敗を再現できると、
  「自分の変更のせいではない＝検査対象の既知の不具合だ」と結論づけたくなる。しかし**テストが
  古くから間違っている場合も、同じようにdefaultブランチで失敗する**。再現できたことが示すのは
  「今回の変更が原因ではない」までであって、原因がどちら側にあるかではない。
  - **切り分けは、検査対象の出力そのものを直接見て行う。** 実例では、hookが出力するJSONを
    ファイルへ落として「文字列値の中に`\r`エスケープが何個あるか」を数えたところ**0個**で、
    CRを持ち込んでいたのは検査対象ではなく**テストが`jq -r`で値を取り出す処理**だと判明した
    （上記「文字コード」節の複数行の値の話）。
  - **CRの有無を数えるときに`grep`/`sed`へバックスラッシュ入りのパターンを直接渡さない。**
    同じ切り分けの中で2度誤った。バックスラッシュ2つで書いたつもりのパターンが**1つに潰れて
    渡り**、grepのBREでは単なる文字`r`として解釈されて無関係な語まで数えた。sedでは同じ潰れ方に
    より、CRを除去するはずの置換が「何も除去しない置換」としてファイルへ書き込まれた。
    **どちらも失敗せず、もっともらしい数値・もっともらしいコードが出る**ため気づけない。
    パターンはヒアドキュメントでファイルへ書き出して`grep -F -f`で渡し、数え上げは
    **除去前後のバイト数の差**で行う（この節を書くときにも同じことが起きた。上の「文字コード」節の
    `awk`/`sed`の項と同じ原因である）。
